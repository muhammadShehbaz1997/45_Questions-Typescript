...
let users = ["Admin", "Shehbaz", "Aftab Bhai"];
users.splice(0);
if (users.length <= 0) {
...
*/
let current_users = ["Shehbaz", "Kamran", "Aftab Bhai", "Osama"];
let new_users = ["Abdul Hadi", "Shehbaz", "Saad Ali", "Osama"];
function existing(name) {
    return current_users.some((value) => value.toLowerCase() === name.toLowerCase());
}
...
I really love pizza!
*/
let favorite_Pizza = ["Hawaiian Pizza", "Meat Pizza", "Margherita Pizza"];
...
make a great pet!
*/
let animals = ["Hippopotamus", "Giraffe", "Wild Boar"];
...
default country.
*/
function describe_city(name, text = "Pakistan") {
...
*/
let make_album = (name, title, track) => {
    return {
        name: name,
        title: title,
        track: track ?? 0
